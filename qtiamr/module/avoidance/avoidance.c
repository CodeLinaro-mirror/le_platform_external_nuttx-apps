...
void register_ultra_client(struct avoid_client * client)
{
 ...
 int avoidance_main(int argc, char *argv[])
 {
	...
	if (ret < 0)
	{
		syslog(LOG_INFO, "avoidance distance parameters get failed\n");
	...
	{
		syslog(LOG_INFO, "ultra disabled\n");
	...
	if ( g_avoid_pid < 0 )
	{
		syslog(LOG_INFO, "create avoid management thread failed\n");
		config_notify_completed(false);
		return ERROR;
	} else {
		syslog(LOG_DEBUG, "create avoid management thread with pid %d\n", g_avoid_pid);
	}
