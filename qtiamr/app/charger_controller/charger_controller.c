...
/* charger management report message type*/
...
/* charger controller reply message type*/
static const char * cmd_type[] = {
  [GET_CTL_VOLTAGE] =      "controller received: get voltage!",
  [GET_CTL_CURRENT] =      "controller received: get current!",
  [GET_CTL_SM_STATE] =     "controller received: get sm state!",
  [GET_CTL_EXCEPTION] =    "controller received: get exception!",
  [GET_CTL_PILE_STATE] =   "controller received: get pile state!",
  [GET_CTL_IS_CHARGING] =  "controller received: get is charging state!",
  [GET_CTL_ALL_STATE] =    "controller received: get all state!",
  [START_CTL_CHARGING] =   "controller received: start charging!",
  [STOP_CTL_CHARGING] =    "controller received: stop charging!",
};
...
static void charger_qrc_msg_send(struct charger_ctl_msg_s *ctl_msg_data){
  ...
  result = qrc_write(g_charger_pipe , (uint8_t *)ctl_msg_data, sizeof(struct charger_ctl_msg_s), false);
  if (result != SUCCESS)
   {
...
static void charger_send_qrc_async_msg_cb(struct charger_msg_s *charger_msg){
    ...
    enum qrc_write_status_e result;
    struct charger_ctl_msg_s controller_qrc_msg_send;
    ...
    memset(&controller_qrc_msg_send, 0, sizeof(struct charger_ctl_msg_s));
    controller_qrc_msg_send.cmd_type = (uint32_t)charger_msg->msg_type;

    switch(controller_qrc_msg_send.cmd_type)
    {
      case KEY_VOLTAGE:
        controller_qrc_msg_send.cmd_data.voltage = charger_msg->data.voltage;
        break;
      case KEY_CURRENT:
        controller_qrc_msg_send.cmd_data.current = charger_msg->data.current;
        break;
      case KEY_SM_STATE:
        controller_qrc_msg_send.cmd_data.sm_state = (uint32_t)charger_msg->data.sm_state;
        break;
      case KEY_EXCEPTION:
        controller_qrc_msg_send.cmd_data.exception_value = (uint32_t)charger_msg->data.exception_err;
         break;
      default:
         break;
   }
    ...
    result = qrc_write(g_charger_pipe, (uint8_t *)&controller_qrc_msg_send, sizeof(struct charger_msg_s), false);
    if (result != SUCCESS)
      {
...
static void charger_controller_msg_parse(struct qrc_pipe_s *pipe, struct charger_ctl_msg_s *control_msg)
{
  ...
  syslog(LOG_INFO, "CHARGER CONTROLLER:charger_controller_msg_parse: cmd_type:%s:(%ld) ", cmd_type[control_msg->cmd_type],control_msg->cmd_type);
  switch(control_msg->cmd_type)
   {
       ...
       case START_CTL_CHARGING:
         memset(&qrc_ctl_msg_send, 0, sizeof(struct charger_ctl_msg_s));
         qrc_ctl_msg_send.cmd_type = START_CTL_CHARGING;
         charger_qrc_msg_send(&qrc_ctl_msg_send);
         sm_start_charging();
         break;
       ...
       case STOP_CTL_CHARGING:
         memset(&qrc_ctl_msg_send, 0, sizeof(struct charger_ctl_msg_s));
         qrc_ctl_msg_send.cmd_type = STOP_CTL_CHARGING;
         charger_qrc_msg_send(&qrc_ctl_msg_send);
         sm_stop_charging();
         break;
