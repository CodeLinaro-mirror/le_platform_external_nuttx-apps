...
static void xmodem_receive(void)
{
  ...
  while (XMODEM_OK == status)
  {
	...
	printf("debug xmodem_receive header= %d recevic_n = %d \n",header,recevic_n);
    ...
    else if ((recevic_n != 1) && (true == g_boot_xmodem.first_packet_received))
    {
		...
		printf("debug xmodem_receive xmodem_error_handler \n");
    }
    else
    {
      /* Do nothing. */
    ...
    switch(header)
    {
		...
		case X_STX:
			...
			{
				status = xmodem_error_handler(&error_number, X_MAX_ERRORS);
				printf("debug xmodem_receive xmodem_error_handler X_MAX_ERRORS  \n");
			}
        break;
      /* End of Transmission. */
      case X_EOT:
...
void bootloader_workflow(void)
{
    ...
    while(1)
    {
		...
		printf("bootloader_workflow resp = %d \n",resp);
        if (1 == recevic_n)
        {
            switch(resp)
            {
